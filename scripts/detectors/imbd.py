...
class IMBD(DetectorBase):
    def __init__(self, config_name):
        super().__init__(config_name)
        self.criterion_fn = get_sampling_discrepancy_analytic
        model_path = path.join(self.config.cache_dir, self.config.scoring_model_name)
        self.scoring_tokenizer = AutoTokenizer.from_pretrained(model_path)
        self.scoring_model = AutoPeftModelForCausalLM.from_pretrained(model_path)
        self.scoring_model.to(self.config.device)
