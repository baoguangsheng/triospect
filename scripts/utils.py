...
def group_by_id(data):
    prefix_idx = {'gen': 3, 'hum': 2, 'rep': 1}
    grouped_items = {}  # id -> [hum item, rep item, hum item, gen item]
    for item in data:
        parts = item['id'].split('/')
        prefix = parts[0]
        hum_id = parts[-1]
        if hum_id not in grouped_items:
            grouped_items[hum_id] = [None, None, None, None]
        idx = prefix_idx.get(prefix, 0)
        grouped_items[hum_id][idx] = item
    # check
    for hum_id in grouped_items:
        items = grouped_items[hum_id]
        assert items[0] is None or items[0]['id'] == hum_id
        assert items[1] is None or items[1]['id'] == f'rep/{hum_id}'
        assert items[2] is None or items[2]['id'] == f'hum/gen/{hum_id}'
        assert items[3] is None or items[3]['id'] == f'gen/{hum_id}'
